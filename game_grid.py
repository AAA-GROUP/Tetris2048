...
class GameGrid:
    ...
    def draw_grid(self, score, next_tetrominoes):
        ...
        stddraw.text(14.5, 15, "Next Shapes")
        stddraw.setPenColor(Color(31, 160, 239))
        stddraw.rectangle(12, 2, 5, 14)
        stddraw.line(12, 8, 17, 8)
        stddraw.line(12, 14, 17, 14)
        stddraw.boldText(19.1, 17, "KEY BINDINGS")
        stddraw.setPenColor(stddraw.MAGENTA)
        stddraw.text(19, 16, "Pause")
        stddraw.line(17.2, 15.6, 20.8, 15.6)
        stddraw.setPenColor(stddraw.BLACK)
        stddraw.text(19, 14.8, "Esc")
        stddraw.setPenColor(stddraw.MAGENTA)
        stddraw.text(19, 13, "Move Tetromino")
        stddraw.line(17.2, 12.6, 20.8, 12.6)
        stddraw.setPenColor(stddraw.BLACK)
        stddraw.text(19, 11.8, "Arrows")
        stddraw.setPenColor(stddraw.MAGENTA)
        stddraw.text(19, 10, "Piece Drop")
        stddraw.line(17.2, 9.6, 20.8, 9.6)
        stddraw.setPenColor(stddraw.BLACK)
        stddraw.text(19, 8.8, "Space")
        stddraw.setPenColor(stddraw.MAGENTA)
        stddraw.text(19, 7, "Rotation")
        stddraw.line(17.2, 6.6, 20.8, 6.6)
        stddraw.setPenColor(stddraw.BLACK)
        stddraw.text(19, 5.8, "R and T")
        stddraw.setPenColor(stddraw.MAGENTA)
        stddraw.text(19, 4, "Toggle Mouse")
        stddraw.line(17.2, 3.6, 20.8, 3.6)
        stddraw.setPenColor(stddraw.BLACK)
        ...
        for i in range(1, height - 1):
            for j in range(1, width - 1):
                if new_tile_matrix[i][j] is not None:
                    if new_tile_matrix[i - 1][j] is None and new_tile_matrix[i][
                        j - 1] is None:  # up and left is not labeled
                        k += 1
                        labeled_grid[i][j] = k  # label initial

                    elif new_tile_matrix[i - 1][j] is not None and new_tile_matrix[i][
                        j - 1] is None:  # up labeled, left non-labeled
                        labeled_grid[i][j] = labeled_grid[i - 1][j]  # label initial with up label

                    elif new_tile_matrix[i - 1][j] is None and new_tile_matrix[i][
                        j - 1] is not None:  # up is not labeled, left is labeled
                        labeled_grid[i][j] = labeled_grid[i][j - 1]  # label initial with left label

                    elif new_tile_matrix[i - 1][j] is not None and new_tile_matrix[i][
                        j - 1] is not None:  # both up and left are labeled
                        if labeled_grid[i - 1][j] == labeled_grid[i][j - 1]:  # up and left labels are equal
                            ...
                            self.__change_label(labeled_grid, labeled_grid[i - 1][j], labeled_grid[i][j - 1], i,
                                                j)  # call change label
